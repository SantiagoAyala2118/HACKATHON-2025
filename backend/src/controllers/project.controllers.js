...
export const createProject = async(req, res)=>{
    ...
    try {
        ...
        if(validatedData.minInvestment < 0 || validatedData.minInvestment > validatedData.fundingGoal)
            res.status(400).json({ok: false, msg: "Por favor, introduzca un monto mínimo válido"})
...
export const getAllProjects = async(req, res)=>{
    ...
    try {
        ...
        if(!projects)
            return res.status(404).json({ok: false, msg: "Aun no tienes ningún proyecto"})
...
export const getProjectByCategory = async(req, res)=>{
    ...
    try {
        ...
        if(!projects) return res.status(404).json({ok: false, msg: "No se encontraron proyectos dentro de esta categoría"})
...
export const deleteProject = async(req, res)=>{
    ...
    try {
        ...
        if(!project) return res.status(404).json({ok: false, msg: "Ese proyecto no existe"})
