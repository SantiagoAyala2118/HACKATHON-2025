...
export const addInvestment = async (req, res) => {
    ...
    try {
        ...
        if (amount < projectFounded.minInvestment)
            return res.status(400).json({ ok: false, msg: `Invierte un monto igual o mayor a $${projectFounded.minInvestment}` });

        if (projectFounded.status === "Financiado")
            return res.status(400).json({ ok: false, msg: "El proyecto ya alcanzó su meta de financiación." });
        ...
        res.status(201).json({ ok: true, msg: `¡Éxito! Has invertido $${amount} para el proyecto ${projectFounded.title}.` });

    } catch (e) {
        console.error(e);
        res.status(500).json({ ok: false, msg: "Error interno del servidor al procesar la inversión." });
    }
};